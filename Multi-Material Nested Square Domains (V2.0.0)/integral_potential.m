function [integral] = integral_potential(side, i, j, ...
...
% delta = 2 * (longSide_length + shortSide_length)  / (0.75*total_nodes) / pr; % The step size for integration points

delta = (2*(longSide_length + shortSide_length) + ...
    2*(longSide_length/2 + shortSide_length/2)) / total_nodes / pr;
...
for k = 1:pr + 1
    integrands(k) = log(1/sum((obPoint-values(:,k)).^2))/(4*pi); % 1.74
end
