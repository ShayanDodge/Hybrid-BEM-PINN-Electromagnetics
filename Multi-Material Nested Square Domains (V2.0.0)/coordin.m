function [boundaryCoords, tangentialUnitVector, normalUnitVector, side] = ...
coordin(longSide_nodes, shortSide_nodes, center, longSide_length, shortSide_length)
...
total_nodes = 2 * (longSide_nodes + shortSide_nodes) + ...
    2 * (longSide_nodes / 2 + shortSide_nodes / 2);
...
delta_long = (2 * (longSide_length + shortSide_length) + ...
    2*(longSide_length/2 + shortSide_length/2)) / total_nodes;

delta_short = (2 * (longSide_length + shortSide_length) + ...
    2*(longSide_length/2 + shortSide_length/2)) / total_nodes;
...
v5 = [-longSide_length/2; -shortSide_length/2] / 2;
v6 = [longSide_length/2 ; -shortSide_length/2] / 2;
v7 = [longSide_length/2 ;  shortSide_length/2] / 2;
v8 = [-longSide_length/2;  shortSide_length/2] / 2;
...
bottom_in_side = zeros(3, longSide_nodes/2);
right_in_side = zeros(3, shortSide_nodes/2);
top_in_side = zeros(3, longSide_nodes/2);
left_in_side = zeros(3, shortSide_nodes/2);
...
% Bottom side (side 5)
for i = 1:longSide_nodes/2
    bottom_in_side(1:2, i) = ((v5 - [delta_long / 2; 0]) + i * [delta_long; 0]);
    bottom_in_side(3, i) = 5; % side index
end

% Right side (side 6)
for i = 1:shortSide_nodes/2
    right_in_side(1:2, i) = ((v6 - [0; delta_short / 2]) + i * [0; delta_short]);
    right_in_side(3, i) = 6; % side index
end

% Top side (side 7)
for i = 1:longSide_nodes/2
    top_in_side(1:2, i) = ((v7 + [delta_long / 2; 0]) - i * [delta_long; 0]);
    top_in_side(3, i) = 7; % side index
end

% Left side (side 8)
for i = 1:shortSide_nodes/2
    left_in_side(1:2, i) = ((v8 + [0; delta_short / 2]) - i * [0; delta_short]);
    left_in_side(3, i) = 8; % side index
end


% Concatenate all sides into one matrix
local_coords = [bottom_side right_side top_side left_side bottom_in_side right_in_side top_in_side left_in_side];
...
for i = 1:total_nodes
    ...
    elseif boundaryCoords(3, i) == 5 % Bottom side
        tangentialUnitVector(:, i) = [-1; 0]; % Tangential vector along x-axis
        normalUnitVector(:, i) = [0; 1]; % Normal vector pointing down
    elseif boundaryCoords(3, i) == 6 % Right side
        tangentialUnitVector(:, i) = [0; -1]; % Tangential vector along y-axis
        normalUnitVector(:, i) = [-1; 0]; % Normal vector pointing right
    elseif boundaryCoords(3, i) == 7 % Top side
        tangentialUnitVector(:, i) = [1; 0]; % Tangential vector along negative x-axis
        normalUnitVector(:, i) = [0; -1]; % Normal vector pointing up
    elseif boundaryCoords(3, i) == 8 % Left side
        tangentialUnitVector(:, i) = [0; 1]; % Tangential vector along negative y-axis
        normalUnitVector(:, i) = [1; 0]; % Normal vector pointing left
    end
end
